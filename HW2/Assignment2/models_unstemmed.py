...
def okapi_tf(query, term_vector, doc_info, avg_doc_length):
    ...
    with open('Files/Unstemmed/Results/OkapiTF_Results_file.txt', 'a+') as results:
        rank = 1
        for ds in doc_score:
...
def bm25(query, term_vector, term_stats, doc_info, avg_doc_length):
    ...
    doc_score.sort(key=itemgetter(1), reverse=True)
    with open('Files/Unstemmed/Results/BM25_Results_file.txt', 'a+') as results:
        rank = 1
        for ds in doc_score:
            results.write('%s Q0 %s %d %lf Exp\n' % (query, ds[0], rank, ds[1]))
            if rank == 1000:
...
def laplace(query, term_vector, doc_info, vocab):
    ...
    doc_score.sort(key=itemgetter(1), reverse=True)
    with open('Files/Unstemmed/Results/laplace.txt', 'a+') as results:
        rank = 1
        for ds in doc_score:
            results.write('%s Q0 %s %d %lf Exp\n' % (query, ds[0], rank, ds[1]))
            if rank == 1000:
...
def main():
    ...
    for query in queries:
        term_stats = unpickler('Files/Unstemmed/Pickles/termStats%s.p' % query)
        term_vector = unpickler('Files/Unstemmed/Pickles/termVector%s.p' % query)
